...
# for (gy in 1:length(groupyears)){
  ...
  SocialCapitalData$DSIgroom = SocialCapitalData$DSIgroom/meanGroomRate
  ...
  SocialCapitalData[,c("numPartnersGroom","between.groom","eig.cent.groom","clusterCoeff.groom")] = 
    NetworkMetrics[match(meta_data$id, NetworkMetrics$id), c("deg","between","eig.cent","clusterCoeff")]
  ...
  SocialCapitalData[,c("numPartnersProx","DSIprox","between.prox","eig.cent.prox")] = 
    NetworkMetrics[match(meta_data$id, NetworkMetrics$id), c("deg","std.strength","between","eig.cent")]
  ...
  #Standerdize
  SocialCapitalData$AggIN=SocialCapitalData$AggIN/mean(SocialCapitalData$AggIN)
  SocialCapitalData$AggOUT=SocialCapitalData$AggOUT/mean(SocialCapitalData$AggOUT)
  ...
  ##################################################################
  #Measure of loneliness - Mismatch between social interest and social attainment; for example:
  
  social_interest = SocialCapitalData$numPartners
  social_attainment = SocialCapitalData$DSIgroom
...
#Z-score regressors.
SocialCapital.ALL[,c(3,5,8:ncol(SocialCapitalData))]=scale(SocialCapital.ALL[,c(3,5,8:ncol(SocialCapitalData))])
...
# #Include all computed parameters
# correl_socialCapital = SocialCapital.ALL[,c(3,5,8:ncol(SocialCapitalData))]


#"Deconstructing Social Brain Model" select a subset which recapitulate the social profile of an individual.
correl_socialCapital = SocialCapital.ALL[,c("age","percentrank","GroomIN","GroomOUT","between.groom","eig.cent.groom",
                                            "AggIN","AggOUT")]
names(correl_socialCapital)=c("age","rank","Groom IN","Groom OUT","betweenness","eig. centrality",
                              "Agg. IN","Agg OUT")

# #"Social integration model" One measure of "objeective" social integration measure and one proxy of "perceived social isolation"
# correl_socialCapital = SocialCapital.ALL[,c("age","percentrank","DSIgroom","loneliness")]
# names(correl_socialCapital)=c("age","rank","Time spent grooming","loneliness")

# #Visualize values for each regressor, for each individual. Are there obvious patterns across IDs?:
# matrix_plot = as.matrix(correl_socialCapital)
