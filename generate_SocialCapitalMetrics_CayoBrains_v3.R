...
# for (gy in 1:length(groupyears)){
  ...
  tiff("NetworkGraph_between.tiff", units="in", width=10, height=8, res=300, compression = 'lzw')
  plot.igraph(am.g,layout=l, vertex.label=V(am.g)$name, vertex.color=V(am.g)$color, vertex.size=3*V(am.g)$between,edge.color="grey20", 
  ...
  el=weightedEL[weightedEL$weight!=0,]
  ...
  for (id in 1:length(groom.ID)){ #For all IDs
    ...
    kin[id,"strengthKin"]=sum(el$weight[which(el$givingID==groom.ID[id]|el$receivingID==groom.ID[id] & el$KinPairClass=="rel")])
    kin[id,"strengthTotal"]=sum(el$weight[which(el$givingID==groom.ID[id]|el$receivingID==groom.ID[id])])
  }
...
# selected_regressors = SocialCapital.ALL[,c("id","sex","age","ordrank","percentrank","GroomOUT","GroomIN",
#                                            "numPartnersGroom","between.groom","eig.cent.groom","clusterCoeff.groom",
#                                            "Kin","propKin","AggOUT","AggIN","tenor","vig.ra","sdb.ra","loneliness")]
# setwd('C:/Users/Camille Testard/Desktop/Desktop_CayoBrains/')
# write.csv(selected_regressors,'social_metrics_kennyOnly.csv')

# # SOCIAL BRAIN v1
# #"Deconstructing Social Brain Model" select a subset which recapitulate the social profile of an individual.
# selected_regressors = SocialCapital.ALL[,c("id","sex","age","ordrank","GroomIN","GroomOUT","between.groom","eig.cent.groom",
#                                             "AggIN","AggOUT","days_between")]
# names(selected_regressors)=c("id","sex","age","rank","Groom IN","Groom OUT","betweenness","eig. centrality",
#                               "Agg. IN","Agg OUT","days between")

# # SOCIAL BRAIN MODEL v2
#Select a subset which recapitulate the social profile of an individual.
# selected_regressors = SocialCapital.ALL[,c("id","sex","age","ordrank","std.DSIgroom","between.groom","eig.cent.groom",
#                                             "clusterCoeff.groom", "closeness.groom","days_between")]
# names(selected_regressors)=c("id","sex","age","rank","Grooming","betweenness","eig. centrality",
#                               "cluster coeff","closeness","days between")

# #AGGRESSION MODEL 
# selected_regressors = SocialCapital.ALL[,c("id","sex","age","ordrank","AggIN","AggOUT","days_between")]
# names(selected_regressors)=c("id","sex","age","rank","Agg IN","Agg OUT","days between")

# #SOCIAL INTEGRATION MODEL
# #One measure of "objective" social integration measure and one proxy of "perceived social isolation"
selected_regressors = SocialCapital.ALL[,c("id","sex","age","ordrank","std.numPartnersGroom","loneliness",
                                           "std.AggIN","days_between")]
names(selected_regressors)=c("id","sex","age","rank","#partners","loneliness","aggression rec."
                             ,"days between")

# #SEX AND AGE MODEL
# selected_regressors = SocialCapital.ALL[,c("id","sex","age","rank")]
# names(selected_regressors)=c("id","sex","age","rank")

#Make sure regressors are in the correct order of subject
setwd('D:/DBM-full/monkeys')
monkey_list = list.dirs(path = ".", full.names = TRUE, recursive = TRUE)
monkey_list=substr(monkey_list[2:68],3,5)
monkey_list_df = data.frame(monkey_list); names(monkey_list_df)="id"

output=merge(monkey_list_df,selected_regressors, by="id")

# setwd('C:/Users/Camille Testard/Desktop/Desktop_CayoBrains')
# write_xlsx(output,'social_metrics2_allBrains.xlsx')

#Format for glm design matrix:
#Format sex column
...
setwd('C:/Users/Camille Testard/Desktop/Desktop_CayoBrains')
write.csv(output,'design_age.csv',row.names = F)
...
ggsave('Social Isolation Model.tiff')
