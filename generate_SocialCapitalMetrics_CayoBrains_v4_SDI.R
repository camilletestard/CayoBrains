...
biobanking_data = biobanking_data[, c("id","sex","age","days_between","weight_kg", "brain_weight_grams", "trapping_date")]
biobanking_data$id = as.character(biobanking_data$id)
...
for (id in 1:length(groom.ID)){ #For all IDs
  ...
  groom.partner[id, "numPartnersIN"] = length(unique(groom_data$groom_giver[groom_data$groom_reciever == groom.ID[id]]))
  groom.partner[id, "numPartnersOUT"] = length(unique(groom_data$groom_reciever[groom_data$groom_giver == groom.ID[id]]))
  groom.partner[id, "overlap"]=length(intersect(unique(groom_data$groom_giver[groom_data$groom_reciever == groom.ID[id]]),
                                         unique(groom_data$groom_reciever[groom_data$groom_giver == groom.ID[id]])))
}
groom.partner$numPartners <- groom.partner$numPartnersIN + groom.partner$numPartnersOUT
...
#NUMBER UNIQUE PARTNERS - adjust number of partner by #hours observed
groom.partner$adjusted.numPartners <- round(groom.partner$numPartners/ meta_data$hrs.focalfollowed, 5)
groom.partner$adjusted.numPartnersIN <- round(groom.partner$numPartnersIN/ meta_data$hrs.focalfollowed, 5)
groom.partner$adjusted.numPartnersOUT <- round(groom.partner$numPartnersOUT/ meta_data$hrs.focalfollowed, 5)
...
SocialCapitalData$numPartnersGroomIN = groom.partner$numPartnersIN
mean_numP = mean(groom.partner$adjusted.numPartnersIN)
SocialCapitalData$std.numPartnersGroomIN = groom.partner$adjusted.numPartnersIN/mean_numP

SocialCapitalData$numPartnersGroomOUT = groom.partner$numPartnersOUT
mean_numP = mean(groom.partner$adjusted.numPartnersOUT)
SocialCapitalData$std.numPartnersGroomOUT = groom.partner$adjusted.numPartnersOUT/mean_numP
...
# ##################################################################
# Sallet Submission-Dominance-Index

interaction_data <- focal_data[focal_data$partner_id!="none", c("focal_id","behaviour","winner","initiator")]
SDI_idx=data.frame(matrix(NA,length(unqIDs), 3)); colnames(SDI_idx)=c("id","total"); id=1
for (id in 1:length(unqIDs)){
  SDI_idx[id,"id"]=unqIDs[id]
  if (length(nrow(interaction_data[interaction_data$focal_id==unqIDs[id],]))!=0){
  SDI_idx[id, "total"] = nrow(interaction_data[interaction_data$focal_id==unqIDs[id],])}
}
SDI_idx$agg.give<-agg.give$Freq[match(SDI_idx$id, agg.give$id)]
SDI_idx$agg.receive<-agg.receive$Freq[match(SDI_idx$id, agg.receive$id)]
SDI_idx$SDI = SDI_idx$agg.give/SDI_idx$total
SDI_idx[is.na(SDI_idx)]<-0
SDI_idx$agg.score <- SDI_idx$agg.give/(SDI_idx$agg.give+SDI_idx$agg.receive)
SDI_idx$std.SDI<- SDI_idx$SDI/mean(SDI_idx$SDI)
SocialCapitalData$SDI<-SDI_idx$std.SDI
SocialCapitalData$agg.score<-SDI_idx$agg.score

Lab_monkeys<-c(81.82,0,8.7,26.09,36.36,42.11,53.85,23.08,57.89,
               17.39,33.33,17.39,0,60.47,21.88,6.67,0,93.33,100,
               59.09,31.25,0,0,71.43,66.67)

plot(density(Lab_monkeys), main="Submission-Dominane Index (SDI) for lab monkeys", xlab = "SDI")
plot(density(SDI_idx$SDI*100), main="Submission-Dominane Index (SDI) for free-ranging monkeys", xlab = "SDI",add =T)

data<- data.frame(SDI= c(SDI_idx$SDI*100, Lab_monkeys),
                  label = c(rep(c("free-ranging"), each = length(SDI_idx$SDI)),rep("laboratory",25)))
ggplot(data, aes(x = SDI, fill = label)) + geom_density(alpha=0.5) + theme_classic(base_size = 18)
...
SocialCapital.ALL[,"brain_wt"] = biobanking_data[match(SocialCapital.ALL$id,biobanking_data$id),"brain_weight_grams"]
...
#                                            "eig.cent.groom","closeness.groom","brain_wt")]
# names(selected_regressors)=c("id","sex","age","rank","SDI","betweenness","eig. centrality","closeness","brain weight")

# selected_regressors = SocialCapital.ALL[,c("id","sex","age","percentrank","agg.score","SDI",
#                                            "std.GroomOUT","std.GroomIN","std.CSIgroom",
#                                            "std.numPartnersGroom","std.numPartnersGroomIN","std.numPartnersGroomOUT","between.groom",
#                                            "eig.cent.groom","closeness.groom","brain_wt")]
# names(selected_regressors)=c("id","sex","age","rank","agg.score","SDI",
#                              "groom strength","groom IN", "groom OUT",
#                              "#partners","#partners IN","#partners OUT",
#                              "betweenness","eig. centrality","closeness","brain weight")

selected_regressors = SocialCapital.ALL[,c("id","sex","age","percentrank","SDI",
                                           "std.numPartnersGroom","std.numPartnersGroomIN","std.numPartnersGroomOUT",
                                           "between.groom","eig.cent.groom","closeness.groom","brain_wt")]
names(selected_regressors)=c("id","sex","age","rank","SDI",
...
write.csv(output,'allVariables_Models_HH2016.csv',row.names = F)
...
#Visualize
ggcorrplot(corr[,11:1], method='circle', lab=T,legend.title=c("corr. coeff."))
